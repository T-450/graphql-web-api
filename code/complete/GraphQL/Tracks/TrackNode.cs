...
namespace ConferencePlanner.GraphQL.Tracks
{
    ...
    public class TrackNode
    {
        ...
        public IQueryable<Session> GetSessions(
            [Parent] Track track,
            [ScopedService] ApplicationDbContext dbContext) => dbContext.Tracks.Where(t => t.Id == track.Id).SelectMany(t => t.Sessions);

        [NodeResolver]
        public static Task<Track> GetTrackByIdAsync(
            int id,
            TrackByIdDataLoader trackByIdDataLoader,
            CancellationToken cancellationToken) => trackByIdDataLoader.LoadAsync(id, cancellationToken);
    }
}
